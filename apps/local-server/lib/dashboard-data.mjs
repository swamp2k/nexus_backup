...
export async function listJobs(db, { limit = 100, state } = {}) {
  ...
  const statement = normalizedState
    ? db.prepare(`${select} WHERE j.state = ? ORDER BY j.created_at DESC, j.id DESC LIMIT ?`).bind(normalizedState, normalizedLimit)
    : db.prepare(`${select} ORDER BY j.created_at DESC, j.id DESC LIMIT ?`).bind(normalizedLimit);
  const result = await statement.all();
  return (result.results ?? []).map(rowToJob);
}

export async function listAgents(db) {
  const result = await db.prepare(`SELECT id, name, enabled, created_at, last_seen_at, version FROM backup_agents ORDER BY id ASC`).all();
...
export async function loadSanitizedAgentConfig(path) {
  ...
  try {
    ...
    if (error?.code === "ENOENT") return emptyConfig(false);
    throw error;
  }

  let value;
  try { value = JSON.parse(raw); }
  catch { return emptyConfig(false, true); }
  if (!isRecord(value)) return emptyConfig(false, true);
  ...
  return {
    ...
    rtorrentEndpoints: sanitizeRtorrentEndpoints(value.rtorrentEndpoints),
  };
}

function emptyConfig(available, invalid = false) {
  return {
    available,
    ...(invalid ? { invalid: true } : {}),
    sources: [], repositories: [], restoreTargets: [], endpoints: [], rtorrentEndpoints: [],
  };
}
...
function sanitizeSources(value) {
  ...
  return value.filter(isRecord).map((source) => ({
    ...
    paths: Array.isArray(source.paths) ? source.paths.filter((path) => typeof path === "string").map((path) => path.trim()).filter(Boolean) : [],
...
function sanitizeRestoreTargets(value) {
  ...
  return value.filter(isRecord).map((target) => {
    ...
    const overwrite = ["always", "if-changed", "if-newer", "never"].includes(target.overwrite) ? target.overwrite : "never";
...
function sanitizeEndpoints(value) {
  ...
  return value.filter(isRecord).map((endpoint) => {
    const mount = isRecord(endpoint.mount)
      ...
      : { enabled: false };
    return { id: stringOrEmpty(endpoint.id), fs: stringOrEmpty(endpoint.fs), allowMove: endpoint.allowMove === true, mount };
  }).filter((endpoint) => endpoint.id);
}

function sanitizeRtorrentEndpoints(value) {
  if (!Array.isArray(value)) return [];
  return value.filter(isRecord).map((endpoint) => ({ id: stringOrEmpty(endpoint.id) })).filter((endpoint) => endpoint.id);
}
...
function rowToJob(row) {
  ...
  const runtime = row.runtime_tool === null || row.runtime_tool === undefined ? null : {
    tool: String(row.runtime_tool), updatedAt: String(row.runtime_updated_at),
    bytesDone: nullableNumber(row.runtime_bytes_done), bytesTotal: nullableNumber(row.runtime_bytes_total),
    filesDone: nullableNumber(row.runtime_files_done), filesTotal: nullableNumber(row.runtime_files_total),
    speedBytesPerSecond: nullableNumber(row.runtime_speed_bytes_per_second), etaSeconds: nullableNumber(row.runtime_eta_seconds),
    errors: nullableNumber(row.runtime_errors),
  };
  return {
    id: String(row.id), operationKey: String(row.operation_key), type: String(row.type), state: String(row.state),
    attempt: Number(row.attempt), revision: Number(row.revision), payload,
    lease: hasLease ? { agentId: String(row.lease_agent_id), acquiredAt: String(row.lease_acquired_at), expiresAt: String(row.lease_expires_at), heartbeatAt: String(row.lease_heartbeat_at) } : null,
    runtime, createdAt: String(row.created_at), updatedAt: String(row.updated_at),
...
function normalizeState(value) { if (typeof value !== "string" || value.length === 0) return null; return JOB_STATES.has(value) ? value : null; }
