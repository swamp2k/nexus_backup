...
export function createTransferRuleService({
  db,
  enqueueJob,
  loadAgentConfig,
  now = () => new Date(),
  ...
  async function create(input) {
    ...
    await db.prepare(`
      INSERT INTO transfer_rules (
        ...
        retry_count,retry_wait_seconds,includes_json,excludes_json,next_scan_at,created_at,updated_at
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).bind(
    ...
    for (const row of rows) {
      try {
        ...
        if (row.job_state === "completed") {
          const cleanupAfter = Number(row.cleanup_days) > 0
...
export async function persistTransferDiscovery(db, {
  ...
  for (const item of normalized.entries) {
    ...
    statements.push(db.prepare(`
      INSERT INTO transfer_discovery_entries(job_id,rule_id,object_key,rel_path,size,mod_time,seen_at)
      VALUES(?,?,?,?,?,?,?)
  ...
  statements.push(db.prepare(`
    ...
    WHERE rule_id=? AND state IN ('discovered','retry_wait')
      AND EXISTS (
        ...
        WHERE s.job_id=? AND s.rule_id=transfer_objects.rule_id
          AND s.rel_path=transfer_objects.rel_path AND s.object_key<>transfer_objects.object_key
      )
  ...
  statements.push(db.prepare(`
    ...
    SELECT rule_id,object_key,rel_path,size,mod_time,seen_at,seen_at,seen_at,?
...
export function normalizeTransferDiscoveryEvent(value, { expectedRuleId, now = new Date() } = {}) {
  if (!isRecord(value) || value.type !== "transfer-discovery" || value.tool !== "rclone") {
    throw new RangeError("transfer discovery event must be a rclone transfer-discovery object");
  }
  ...
  const entries = value.entries.map(normalizeDiscoveryEntry);
  const seen = new Set();
  for (const entry of entries) {
    if (seen.has(entry.relPath)) throw new RangeError(`duplicate transfer discovery path: ${entry.relPath}`);
...
export async function normalizeRuleInput(value, { loadAgentConfig } = {}) {
  ...
  const result = {
    ...
    sourcePath: normalizeRelativeBase(value.sourcePath ?? ""),
    destinationEndpointId: requireId(value.destinationEndpointId, "destinationEndpointId"),
    destinationPath: normalizeRelativeBase(value.destinationPath ?? ""),
    ...
    retryCount: integerRange(value.retryCount ?? 3, "retryCount", 0, 20),
    retryWaitSeconds: integerRange(value.retryWaitSeconds ?? 300, "retryWaitSeconds", 0, 86400),
    includes: normalizePatterns(value.includes),
    excludes: normalizePatterns(value.excludes),
  };
  if (result.sourceEndpointId === result.destinationEndpointId && result.sourcePath === result.destinationPath) {
    throw new RangeError("transfer source and destination must be different");
  }
  if (typeof loadAgentConfig === "function") {
    ...
    const source = endpoints.get(result.sourceEndpointId);
    const destination = endpoints.get(result.destinationEndpointId);
...
function rowToRule(row, counts) {
  ...
  return {
    ...
    mode: String(row.mode), initialBehavior: String(row.initial_behavior),
    stabilitySeconds: Number(row.stability_seconds), scanIntervalSeconds: Number(row.scan_interval_seconds),
    cleanupDays: Number(row.cleanup_days), verification: String(row.verification),
    retryCount: Number(row.retry_count), retryWaitSeconds: Number(row.retry_wait_seconds),
