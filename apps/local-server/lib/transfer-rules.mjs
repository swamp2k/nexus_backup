...
const READINESS = new Set(["stability", "rtorrent_complete", "rtorrent_incomplete"]);
...
export function createTransferRuleService({ db, enqueueJob, loadAgentConfig, now = () => new Date(), id = () => randomUUID() }) {
  ...
  async function create(input) {
    ...
    await db.prepare(`
      INSERT INTO transfer_rules (
        ...
        includes_json,excludes_json,rtorrent_endpoint_id,rtorrent_required,next_scan_at,created_at,updated_at
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).bind(
      ...
      value.rtorrentEndpointId ?? null, value.rtorrentRequired ? 1 : 0,
  ...
  async function update(ruleId, input) {
    ...
    const structuralChange = value.sourceEndpointId !== existing.sourceEndpointId
      ...
      || value.rtorrentEndpointId !== existing.rtorrentEndpointId
      || value.rtorrentRequired !== existing.rtorrentRequired
    ...
    if (structuralChange) {
      if (existing.lastScanJob && ACTIVE_STATES.has(existing.lastScanJob.state)) throw statusError(409, "Transfer rule cannot change endpoints/readiness/filters during an active scan");
      if ((await activeManagedJobs(db, existing.id)).length > 0) throw statusError(409, "Transfer rule cannot change endpoints/readiness/filters while transfers are active");
    }
    ...
    await db.batch([
      db.prepare(`
        UPDATE transfer_rules SET
          ...
          mode=?,initial_behavior=?,stability_seconds=?,scan_interval_seconds=?,cleanup_days=?,verification=?,
          multi_thread_streams=?,multi_thread_cutoff=?,retry_count=?,retry_wait_seconds=?,rclone_args_json=?,
          includes_json=?,excludes_json=?,rtorrent_endpoint_id=?,rtorrent_required=?,next_scan_at=?,updated_at=?,revision=revision+1,
      ...
      `).bind(
        ...
        value.cleanupDays, value.verification, value.multiThreadStreams, value.multiThreadCutoff,
        value.retryCount, value.retryWaitSeconds, JSON.stringify(value.rcloneArgs),
        JSON.stringify(value.includes), JSON.stringify(value.excludes), value.rtorrentEndpointId ?? null,
        value.rtorrentRequired ? 1 : 0, value.enabled ? at.toISOString() : null, at.toISOString(),
        structuralChange ? 1 : 0, existing.id,
      ),
  ...
  async function queueDiscovery(rule, at, manual) {
    ...
    const job = await enqueueJob({
      ...
      payload: {
        ...
        ...(rule.rtorrentEndpointId ? { rtorrentEndpointId: rule.rtorrentEndpointId } : {}),
        rtorrentRequired: rule.rtorrentRequired,
      },
    });
    ...
    const rows = (await db.prepare(`
      ...
      WHERE r.enabled=1 AND (
        (o.state='discovered'
          ...
          AND (
            o.readiness='rtorrent_complete'
            OR (o.readiness='stability' AND julianday(o.stable_since)<=julianday(?)-(CAST(r.stability_seconds AS REAL)/86400.0))
          ))
...
export async function persistTransferDiscovery(db, { jobId, expectedRuleId, event, at = new Date() }) {
  ...
  for (const item of normalized.entries) {
    ...
    statements.push(db.prepare(`
      INSERT INTO transfer_discovery_entries(
        job_id,rule_id,object_key,rel_path,size,mod_time,seen_at,readiness,torrent_hash,torrent_name,torrent_root
      ) VALUES(?,?,?,?,?,?,?,?,?,?,?)
    `).bind(
      jobId, normalized.ruleId, key, item.relPath, item.size, item.modTime, seenAt,
      item.readiness, item.torrentHash ?? null, item.torrentName ?? null, item.torrentRoot ?? null,
    ));
  }
  statements.push(db.prepare(`
  ...
  statements.push(db.prepare(`
    INSERT INTO transfer_objects(
      rule_id,object_key,rel_path,size,mod_time,first_seen_at,last_seen_at,stable_since,state,
      readiness,torrent_hash,torrent_name,torrent_root
    )
    SELECT rule_id,object_key,rel_path,size,mod_time,seen_at,seen_at,seen_at,?,readiness,torrent_hash,torrent_name,torrent_root
    FROM transfer_discovery_entries WHERE job_id=?
    ON CONFLICT(rule_id,object_key) DO UPDATE SET
      ...
      torrent_root=excluded.torrent_root
...
export function normalizeTransferDiscoveryEvent(value, { expectedRuleId, now = new Date() } = {}) {
  ...
  const rtorrent = normalizeRtorrentStatus(value.rtorrent);
  return { type: "transfer-discovery", tool: "rclone", ruleId, at: normalizeAt(value.at, now), rtorrent, entries };
}
...
export async function normalizeRuleInput(value, { loadAgentConfig } = {}) {
  ...
  const rtorrentEndpointId = optionalId(value.rtorrentEndpointId, "rtorrentEndpointId");
  const rtorrentRequired = value.rtorrentRequired === undefined ? false : requireBoolean(value.rtorrentRequired, "rtorrentRequired");
  if (rtorrentRequired && !rtorrentEndpointId) throw new RangeError("rtorrentRequired needs rtorrentEndpointId");
  const result = {
    ...
    rtorrentEndpointId, rtorrentRequired,
  };
  ...
  if (typeof loadAgentConfig === "function") {
    ...
    if (result.rtorrentEndpointId) {
      const rtorrentIds = new Set((config.rtorrentEndpoints ?? []).map((item) => item.id));
      if (!rtorrentIds.has(result.rtorrentEndpointId)) throw new RangeError(`unknown rtorrentEndpointId: ${result.rtorrentEndpointId}`);
    }
  }
  return result;
}
...
function normalizeDiscoveryEntry(value) {
  ...
  const readiness = value.readiness === undefined ? "stability" : requireEnum(value.readiness, READINESS, "entry.readiness");
  const torrentHash = optionalString(value.torrentHash, "entry.torrentHash", 1, 128);
  const torrentName = optionalString(value.torrentName, "entry.torrentName", 1, 512);
  const torrentRoot = value.torrentRoot === undefined || value.torrentRoot === null ? undefined : normalizeObjectPath(value.torrentRoot);
  if (readiness === "stability" && (torrentHash || torrentName || torrentRoot)) throw new RangeError("stability discovery entries may not contain torrent metadata");
  return {
...
function optionalId(value,name){if(value===undefined||value===null||value==="")return null;return requireId(value,name)}
function optionalString(value,name,min,max){if(value===undefined||value===null||value==="")return undefined;return requireString(value,name,min,max)}
