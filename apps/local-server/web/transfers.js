...
function ruleCard(rule){
  ...
  return `<section class="card transfer-card" data-rule="${attr(rule.id)}">
    ...
    <div class="transfer-meta"><span>Scan ${duration(rule.scanIntervalSeconds)}</span><span>Stable ${duration(rule.stabilitySeconds)}</span><span>${rule.initialBehavior==="ignore_existing"?"Ignore existing on first scan":"Process existing"}</span><span>Retry ${rule.retryCount} × ${duration(rule.retryWaitSeconds)}</span><span>Size verify</span><span>Cleanup ${rule.cleanupDays?`${rule.cleanupDays}d policy-only`:"off"}</span></div>
    <div class="transfer-meta filters"><span>Include: ${rule.includes.length?esc(rule.includes.join(", ")):"all"}</span><span>Exclude: ${rule.excludes.length?esc(rule.excludes.join(", ")):"none"}</span><span>${rule.lastScanCompletedAt?`Last scan ${relative(rule.lastScanCompletedAt)}`:"Never scanned"}</span><span>${total} tracked generations</span></div>
