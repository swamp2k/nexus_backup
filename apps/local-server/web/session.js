...
function installWorkstationDashboard(){
  ...
  function render(){
    ...
    content.innerHTML=`<div id="workstations-view">
      <div class="transfer-hero"><div><p class="eyebrow">Endpoint backup</p><h2>Workstations</h2><p>Windows PCs copy selected folders into their assigned Repository as ordinary browsable files.</p></div><span class="badge success">Flat files</span></div>
      ...
      <div class="transfer-note"><strong>Data path:</strong> workstation → selected Repository → workstation folder. Missing source files are never deleted from the backup.</div>
    </div>`;
    bind();
  }
  ...
  function card(ws){
    ...
    const storage=status.repositoryConfigured?`${esc(status.repositoryKind||"flat-file")}`:"Repository not configured";
    ...
    return `<section class="card workstation-card" data-ws="${attr(ws.id)}">
      ...
      <div class="transfer-facts">${fact("Storage",storage,status.repositoryConfigured?"Ordinary files under /backup":"Select a repository in policy",status.repositoryConfigured?"":"warn")}${fact("Last success",status.lastSuccessAt?relative(status.lastSuccessAt):"Never",status.lastSnapshotId?`Copy ${String(status.lastSnapshotId).slice(0,12)}`:"Flat-file copy")}${fact("Next run",ws.policy?.nextRunAt?relative(ws.policy.nextRunAt):"Not scheduled",ws.policy?.enabled?`${scheduleLabel(ws.policy.schedule)} · ${ws.policy.timezone}`:"Policy disabled")}${fact("Last run",backupState,run?.error||status.lastError||"No error",run?.state==="failed"||run?.state==="partial"?"danger":"")}</div>
  ...
  async function openEnroll(){
    ...
    modal.querySelector("form").addEventListener("submit",async event=>{
      ...
      try{const form=new FormData(event.currentTarget);const data=await request("/v1/local/workstations/enroll",{method:"POST",body:{name:form.get("name"),repositoryId:form.get("repositoryId"),destinationFolder:form.get("destinationFolder")||undefined}});closeModal();showInstaller(data);await refresh(true)}
  ...
  async function openSources(ws){
    ...
    modal.innerHTML=`<section class="modal ws-policy-modal" role="dialog" aria-modal="true"><div class="modal-header"><div><p class="eyebrow">${esc(ws.name)}</p><h2>Backup sources</h2></div><button class="icon-button" data-ws-close>×</button></div>
      ...
      <div class="transfer-note" data-source-status>${!supported?"Update the workstation client to enable source scanning.":"Loading latest saved scan…"}</div>
