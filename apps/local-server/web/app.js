...
function renderOverview() {
  ...
  content.innerHTML = `<div class="grid metrics">${metricCard("Active jobs", active, active ? "Work in progress" : "Nothing running", "blue")}${metricCard("Queued", queued, queued ? "Waiting for local execution" : "Queue is clear", queued ? "warn" : "")}${metricCard("Success rate", `${successRate}%`, "Last 20 finished jobs", failed ? "warn" : "")}${metricCard("Workstations", "Available", "Flat-file clients", "")}</div><section class="card section-gap"><div class="card-header"><div><h2>Recent jobs</h2><p>Live state from local SQLite</p></div><button class="button ghost" data-go-jobs>View all</button></div>${jobsTable(state.jobs.slice(0, 8))}</section><div class="grid three-col section-gap">${summaryEntityCard("Sources", state.config.sources?.length ?? 0, "Configured source paths", "⇤", "sources")}${summaryEntityCard("Destinations", state.config.endpoints?.length ?? 0, "Configured rclone destinations", "⇥", "destinations")}${summaryEntityCard("Repositories", "→", "Ordinary folders under /backup", "▣", "repositories")}</div>`;
  bindJobRows();
