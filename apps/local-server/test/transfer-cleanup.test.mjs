...
async function fixture(){
  ...
  await db.prepare(`INSERT INTO transfer_objects(rule_id,object_key,rel_path,size,mod_time,first_seen_at,last_seen_at,stable_since,state,committed_at,destination_rel_path,cleanup_after) VALUES('rule',?,'show/episode.mkv',42,?,'2026-09-12T10:00:00.000Z','2026-09-12T10:00:00.000Z','2026-09-12T10:00:00.000Z','done','2026-09-12T10:00:00.000Z','show/episode.mkv','2026-09-19T10:00:00.000Z')`).bind("a".repeat(64),expectedModTime).run();
...
test("due cleanup queues an agent job with size+mtime provenance and marks object cleaned only after completion",async()=>{
  const f=await fixture();try{
    let result=await f.service.runDue();assert.equal(result.queued,1);assert.equal(f.queued[0].type,"managed-cleanup");assert.equal(f.queued[0].payload.expectedSize,42);assert.equal(f.queued[0].payload.expectedModTime,expectedModTime);
...
test("transient cleanup failure retries with the rule retry budget",async()=>{
  const f=await fixture();try{
    ...
    f.setNow("2026-09-20T10:05:06.000Z");result=await f.service.runDue();assert.equal(result.queued,1);assert.match(f.queued.at(-1).operationKey,/attempt:2$/);assert.equal(f.queued.at(-1).payload.expectedModTime,expectedModTime);
