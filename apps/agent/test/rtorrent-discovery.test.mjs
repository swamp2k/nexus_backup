...
const COMPLETE_HASH="a".repeat(40);
const INCOMPLETE_HASH="b".repeat(40);
const XML=`<?xml version="1.0"?><methodResponse><params><param><value><array><data>
<value><array><data><value><string>${COMPLETE_HASH}</string></value><value><string>Complete</string></value><value><i8>1</i8></value><value><string>/srv/rtorrent/complete/Show.Complete</string></value></data></array></value>
<value><array><data><value><string>${INCOMPLETE_HASH}</string></value><value><string>Downloading</string></value><value><i8>0</i8></value><value><string>/srv/rtorrent/complete/Show.Downloading</string></value></data></array></value>
...
test("known incomplete torrent paths are held back while complete torrents become compact groups",async()=>{
  ...
  try{
    ...
    const groupEvent=events.find(event=>event.type==="transfer-groups");
    assert.deepEqual(groupEvent.groups,[{kind:"torrent",key:COMPLETE_HASH,name:"Complete",root:"Show.Complete"}]);
    const summary=events.find(event=>event.type==="summary");
    assert.equal(summary.data.rtorrent.blockedIncompleteFiles,1);
    assert.equal(summary.data.rtorrent.complete,1);
    assert.equal(summary.data.rtorrent.groups,1);
...
test("optional rtorrent outage clears groups and falls back to the stability listing",async()=>{
  ...
  try{
    ...
    const groupEvent=events.find(event=>event.type==="transfer-groups");
    assert.deepEqual(groupEvent.groups,[]);
