...
export class ResticRestoreExecutor implements JobExecutor {
  ...
  async execute(job: BackupJob, signal: AbortSignal): Promise<JobExecutionResult> {
      ...
      stdout: (line) => {
        ...
        if (action === "restored" || action === "updated") {
          if (changedLogs < MAX_CHANGED_LOGS) {
            ...
            changedLogs += 1;
          } else changedLogsTruncated = true;
        } else if (/^Summary:/i.test(line.trim())) {
          this.#events.emit({ type: "log", tool: "restic", stream: "stdout", message: line.slice(0, 12_000) });
        }
      },
      stderr: (line) => {
    ...
    this.#events.emit({ type: "log", tool: "restic", stream: "stdout", message: `Restore complete: ${restored} new · ${updated} updated · ${unchanged} unchanged` });
    return { status: "completed" };
  }
}

function parsePayload(value: unknown) {
  if (!isRecord(value)) throw new Error("restore payload must be an object");
...
function restoreAction(line: string): "restored" | "updated" | "unchanged" | null { const raw = line.trimStart().match(/^(restored|updated|unchanged)\s+/i)?.[1]; if (!raw) return null; const action = raw.toLowerCase(); return action === "restored" || action === "updated" || action === "unchanged" ? action : null; }
function resticEnvironment(repository: LocalResticRepository): Record<string, string> { const env: Record<string, string> = { ...(repository.environment ?? {}), RESTIC_REPOSITORY: repository.repository }; if (repository.passwordFile) env.RESTIC_PASSWORD_FILE = repository.passwordFile; return env; }
function requireSnapshotId(value: unknown): string { const id = requireString(value, "snapshotId", 8, 64); if (!/^[A-Fa-f0-9]{8,64}$/.test(id)) throw new Error("snapshotId must be a hexadecimal Restic snapshot id"); return id; }
function requireSnapshotPath(value: unknown): string { if (typeof value !== "string" || value.length < 1 || value.length > 4096) throw new Error("path must be 1-4096 characters"); if (!value.startsWith("/")) throw new Error("snapshot path must be absolute"); if (value.includes("\0")) throw new Error("snapshot path contains an invalid character"); if (value.split("/").some((segment) => segment === "." || segment === "..")) throw new Error("snapshot path may not contain dot segments"); return value.length > 1 ? value.replace(/\/+$/, "") || "/" : "/"; }
function requireString(value: unknown, name: string, min: number, max: number): string { if (typeof value !== "string") throw new Error(`${name} must be a string`); const normalized = value.trim(); if (normalized.length < min || normalized.length > max) throw new Error(`${name} must be ${min}-${max} characters`); return normalized; }
